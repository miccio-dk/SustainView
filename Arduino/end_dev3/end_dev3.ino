...
float temp1, pres1, lum1;
uint8_t pinSetting1[] = {LDR_PIN, 400};
uint8_t payload[13];


GenericSensor sensor1(MPL115A2, NULL);
GenericSensor sensor2(LDR, pinSetting1);
...
void loop() {
	...
	if ((currentTime-previousTime) > samplingTime) {
		...
		payload[0] = (PL_NODE | PL_TEMPERATURE | PL_PRESSURE | PL_LIGHT);

		if(sensor1.readValue(TEMPERATURE, &temp1)) Serial.print("\nMPL data acq. succesful: ");
		packFloat(temp1, payload+1);
		Serial.println(temp1, 2);

		if(sensor1.readValue(PRESSURE, &pres1)) Serial.print("MPL data acq. succesful: ");
		packFloat(pres1, payload+5);
		Serial.println(pres1, 2);

		if(sensor2.readValue(LIGHT, &lum1)) Serial.print("LDR data acq. succesful: ");
		packFloat(lum1, payload+9);
		Serial.println(lum1);
